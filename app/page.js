...
export default function Home() {
  ...
  const sections = [
    ...
    },
    {
      title: "",
      content: "",
      image: "https://media.wired.com/photos/5af38a6114823b60460cfe67/master/w_1600%2Cc_limit/lidarclip.gif",
      width: 600,
      height: 600,
      right: false,
    },
      ...
      >
        ...
        {sections.map((section, index) => (
          <div key={index} className="mb-8"> 
            ...
            <motion.div variants={itemVariants} className={`flex items-start ${section.right ? 'flex-row-reverse' : ''} space-x-4`}>
              <div className={`flex-1 ${section.image ? 'w-1/2' : 'w-full'}`}> 
                <p className="text-white text-lg">{section.content}</p>
              </div>
              {section.image && (
                <div className={`${section.right ? 'mr-4' : 'ml-4'} ${section.title ? 'mt-0' : 'mt-8'}`}>
                  <Image
                    src={section.image}
